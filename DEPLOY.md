...
| `MONITOR_TOKEN` | Token do health check central da Hack Tech Farm (`GET /api/v1/health/summary`, `Authorization: Bearer <token>`, ADR-0039). Gere: `openssl rand -hex 32` | **Rota responde 503 (sem corpo)** — nunca libera por omissão; o resto da API não é afetado (boot não falha). Token errado/ausente no cabeçalho: 401 sem corpo. |
| `FAL_ADMIN_KEY` (opcional) | Chave **ADMIN** da fal.ai, só para o health ler o custo do mês (`GET https://api.fal.ai/v1/models/usage`). NÃO é a `FAL_KEY` de geração | Sem ela o check `fal_ai` fica **degraded** ("fal.ai não verificada") e mostra só uma estimativa (imagens × `FAL_IMAGE_COST_USD`, padrão US$ 0,03). |
| `FAL_MONTHLY_CAP_USD`, `R2_STORAGE_CAP_GB`, `STRIPE_WEBHOOK_MAX_SILENCE_HOURS`, `FAL_IMAGE_COST_USD` (opcionais) | Tetos e limiares do health: custo mensal da fal.ai (US$), ocupação do R2 (GB) — acima de 80% vira degraded —, horas sem evento do Stripe (padrão 24), preço por imagem da estimativa | Sem teto configurado o check mostra o número e diz "teto não configurado" (fica ok). |
