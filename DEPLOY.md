...
| `NODE_ENV=production` | Já definido no `Dockerfile` — **não sobrescreva** | É o que faz o código ignorar as flags de dev (seção 3.3) **e** exigir `AUTH_SECRET`, `DATABASE_URL` e o R2 (regras acima) válidos no boot. Sem ele a API roda em modo "dev" (segredo padrão inseguro, dados em memória e imagens em disco local incluídos). |
