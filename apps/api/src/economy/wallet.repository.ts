...
export class DrizzleWalletRepository extends WalletRepository {
  ...
  async get(owner: string): Promise<Wallet> {
    ...
    if (rows[0]) return { catalisadores: rows[0].catalisadores, biomassa: rows[0].biomassa, lastDaily: rows[0].lastDaily ?? null, lastBiweekly: rows[0].lastBiweekly ?? null, imageCredits: rows[0].imageCredits ?? 0 };
