                ...
                <div key={e.id} className="rounded-card border border-cyan/20 bg-bg-800 p-3">
                  ...
                  {estado === "NASCIDO" && (
                    <>
                      <div className="relative mx-auto mb-2 grid aspect-square w-full max-w-[180px] place-items-center overflow-hidden rounded bg-bg-900">
                        {(() => {
                          // Lista da incubadora: miniatura (fallback: original) e carregamento preguiçoso — ADR-0037.
                          const img = pickListImage(e.imageUrl, e.thumbUrl, true);
                          return img
                            ? <img key={img.src} src={img.src} alt={displayName(e)} loading="lazy" decoding="async" onError={(ev) => { swapToFallback(ev.currentTarget, img.fallback); }} className="h-full w-full object-cover" />
                            : <span className="font-mono text-[0.6rem] uppercase text-ink-muted">modo procedural</span>;
                        })()}
                      </div>
