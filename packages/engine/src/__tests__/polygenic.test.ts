...
describe("QTL poligênico (ADR-0012)", () => {
  ...
  it("determinístico sob seed", () => {
    const a = cross(mk(0.9, "M") as any, mk(0.3, "F") as any, "F1", "s1", ctx);
    const b = cross(mk(0.9, "M") as any, mk(0.3, "F") as any, "F1", "s1", ctx);
  ...
  it("irmãos variam (segregação) em torno da média parental", () => {
    const portes = ["a","b","c","d","e"].map((s) => cross(mk(0.9, "M") as any, mk(0.3, "F") as any, "F1", s, ctx).specimen.genotype.qtl.porte ?? 0);
  ...
  it("QTL fica no intervalo [0,1]", () => {
    for (const s of ["x","y","z"]) {
      const p = cross(mk(0.98, "M") as any, mk(0.95, "F") as any, "F1", s, ctx).specimen.genotype.qtl.porte;
